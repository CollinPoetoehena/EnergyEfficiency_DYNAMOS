...
In the main branch more experiment files are present, such as calculating the mean, generating box plots, etc. Move the folders with the experiments to main to analyze that data. This is done in this way to avoid having to update the experiment Python files in every branch each time and allowing for a central place to analyze the data. This implementation branch therefore just serves as a branch for the implementation changes and executing the experiments. The main branch contains the files to analyze the data.